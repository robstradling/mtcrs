...
4. **Relying-party verification.**
   A relying party reads the tick and the committed anchor from the certificate, hashes `tick.value` forward `tick.period` times, and checks that the result equals the anchor ({{verification}}).
   Hashing `h[3]` twice yields `h[5]`.
   It also checks that `tick.period` agrees with its own clock to within one period.
   Verification is entirely offline.
   The relying party fetches nothing ({{rp-no-fetch}}).
...
Bounding the field at 16 bits caps that work at 65,535 hash evaluations for any certificate from any CA, and does so in the parser rather than by CA compliance.
A `tick.period` read from a 16-bit field cannot exceed 65,535 whatever the issuer did.
...
A relying party verifies offline and has no provisioning relationship with the CA, so reading the value from the certificate is the only way it can obtain it without a new authenticated distribution channel.
Committing it to the tree makes it self-authenticating.
...
1. Generate a seed of HASH_SIZE bytes (32 bytes for SHA-256) using a cryptographically secure random number generator ({{?RFC4086}}) or an approved deterministic random bit generator.
   ...
   An adversary who can guess or recover it can compute the whole hash chain and forge ticks for a revoked certificate, so it carries the same unpredictability and confidentiality requirement as a secret key ({{seed-confidentiality}}).
...
Backdating by one `tick_interval` or more places it in period 1 or later at the moment of issuance, which forfeits the period 0 grace ({{period-zero-rationale}}) in both its parts.
The CA no longer has until period 1 to serve the first secret tick, and the authenticating party can no longer present the committed anchor while it installs.
...
{{period-zero-rationale}} gives the rationale.

## Revoking a Certificate {#revoking}

To revoke a certificate, the CA stops revealing new values.
Once the previous value expires (at the start of the next period), the certificate is effectively revoked.
No party can produce a valid value without knowledge of unrevealed hash chain elements, which requires inverting the hash function.
...
: The issuing CA's ID, which is its trust anchor ID.
  ...
  Taking it from the certificate rather than from the verifier's own trust anchor keeps this input a property of the certificate, so it does not depend on path-building state.
  It also gives the authenticating party a source, since it has no trust anchor of its own to consult.
...
~~~
{: #fig-hashchaininput title="HashChainInput byte layout for the test vectors, with SHA-256 as HASH. The 45 bytes, together with SHA-256's 9 bytes of padding, occupy a single 64-byte compression block"}

The chain is salted per CA rather than per certificate.
`issuer_ca_id` keeps one CA's hash chains in a domain distinct from every other CA's, and within a CA the independent random seed of {{construction}} is what separates one certificate's chain from another's.

A per-entry salt was considered and rejected.
...
: The tick interval in seconds for this certificate ({{construction}}), with a default of 3600 (one hour).
  ...
  It keeps `tickInterval` a fixed-width integer, so a relying party never has to allocate or evaluate an arbitrarily large INTEGER before reaching the checks that would reject it.
...
4. Check that `tick.period` lies within the *acceptance window*.
   ...
   No tick exists for a period the CA never reveals, and one cannot be forged ({{hash-function-requirements}}), so admitting the value costs nothing.
...
: The base URL MAY additionally be published in the CA's certificate representation ({{Section 5.5 of !I-D.ietf-plants-merkle-tree-certs}}) using the id-ad-mtcrsTicks Subject Information Access access method defined in {{iana-considerations}}, whose `accessLocation` is a `uniformResourceIdentifier` giving the tick base URL.
  ...
  Where a provisioning binding exists (for example ACME, {{acme-integration}}) it is redundant with that channel, which takes precedence when the two disagree.
  Where none exists, it is the only interoperable carrier of the base URL.
  It carries only the base URL, so it does not apply when unguessable tick URLs ({{unguessable-urls}}) are used.
  In that case the full per-certificate URL is delivered through the provisioning channel and the SIA SHOULD NOT be published.
...
The token is correspondingly a bearer capability.
An authenticating party could disclose it, and an on-path observer of a plain-HTTP fetch can see it.
...
2. Before installing a fetched tick, the authenticating party MUST verify it against the anchor committed in its own certificate: that hashing `tick.value` forward `tick.period` times yields the anchor ({{verification}}).
   ...
   The authenticating party SHOULD additionally check that `tick.period` is the period it currently expects, so that an unexpectedly stale or future-dated response is also rejected.
   Because a stale-but-genuine tick still hashes to the anchor, only this freshness check detects it.
...
Such derivation MUST use a keyed KDF or PRF, for example HKDF or HMAC.
A raw `Hash(ca_seed || ...)` construction MUST NOT be used, as it invites length-extension and MAC-misuse and can make derived seeds distinguishable from independent random seeds ({{derived-seeds}}).

If the CA's seed storage is compromised, the CA MUST revoke all affected certificates via the base MTC revocation mechanism (revoked ranges of serial numbers) as a fallback, as described in {{interaction-with-base-mtc-revocation}}.
...
A certificate is considered revoked if either mechanism indicates revocation.
...
Revocation in this mechanism is the *absence* of a tick.
The CA stops revealing hash chain values ({{revealing-values}}) and the certificate becomes unusable within two periods.
...
- **No revocation reason codes.**
  ...
  Conveying a reason is out of scope here.
  A deployment that needs machine-readable reasons uses the base MTC revoked-ranges mechanism or an external revocation system that carries them.
...
That tick is a fresher non-revocation proof than the verifier expected, and a tick for a period the CA has not yet reached cannot be forged, by preimage resistance ({{hash-function-requirements}}).
...
This section collects them for convenience.
Each is specified in full in the section cited, and nothing here is a new requirement.
...
: A relying party that also supports the base MTC revoked-ranges mechanism SHOULD check both.
  A certificate is revoked if either indicates so ({{interaction-with-base-mtc-revocation}}).
...
Each step computes h\[i\] = SHA-256(HashChainInput(h\[i-1\])), where HashChainInput(preimage) is the concatenation label || `issuer_ca_id` || preimage.
For example, HashChainInput(h\[0\]) is the following 45 bytes, which with SHA-256's 9 bytes of padding occupies a single 64-byte compression block:

~~~
6372732f76310a000481fd5901000102030405060708090a0b0c0d0e0f101112
...
h[1]  059d23b611e311ce3ab76ccae5a12d175b6aede9d7d537629a6f2f7ebfbae7ec
h[2]  085a8899525433f5323153579a50c2d254863f2cd21dee45e7da53def37fde76
h[3]  c6936c83dd5ec7b4e5c5f7c643d04200311e2ad8adb4e912623b37213c08f397
h[4]  169f23ce127f6af4ef400c4de2c92915eaa4652724bbbd96b288a1612b3210fb
h[5]  5905f7a677c88b43c7756c40749a557d90c0118de0d5ab50cd71c5713063dc3c
~~~
...
~~~
v0 = h[3] = c6936c83...3c08f397
v1 = SHA-256(HashChainInput(v0)) = 169f23ce...2b3210fb  (= h[4])
...
~~~
30220420
5905f7a677c88b43c7756c40749a557d90c0118de0d5ab50cd71c5713063dc3c
~~~
...
- **The tick is self-authenticating, so an OCSP signature is pointless or harmful.**
  ...
  Signing each tick reintroduces exactly the per-response signing load this mechanism is designed to avoid, as with the OCSP-like per-certificate signatures alternative below ({{operational-resilience}}).
  Leaving it unsigned is not really an OCSP response.

- **It is heavier and less cacheable than a plain GET.**
  The HTTP interface is a static, plain-HTTP, CDN-cacheable key-value fetch with no per-request cryptography.
  OCSP adds DER request construction, a responder, nonce handling, and signature validation, contradicting the operational-simplicity argument ({{operational-resilience}}).
...
- **No criticality lever.**
  ...
  The transition strategy of marking the X.509 extension critical to force unaware relying parties to hard-fail ({{extension-criticality}}) is therefore unavailable.
  Enforcement rests entirely on relying parties that implement this mechanism.
